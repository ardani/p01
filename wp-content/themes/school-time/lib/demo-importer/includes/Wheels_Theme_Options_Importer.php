...
class Wheels_Theme_Options_Importer implements Wheels_Importer_Interface {
	...
	protected $theme_option_name = SCHOOL_TIME_THEME_OPTION_NAME;
	protected $filename;
	...
	public function import() {
		...
		if ( ! file_exists( $this->get_filename() ) ) {
			wp_die( esc_html__( 'Theme options Import file could not be found. Please try again.', 'school-time' ), '', array( 'back_link' => true ) );
		}
		...
		if ($data) {
			...
			if ( empty( $data ) || ! is_array( $data ) ) {
				wp_die( esc_html__( 'Theme options import data could not be read. Please try a different file.', 'school-time' ), '', array( 'back_link' => true ) );
			}
			...
			if ( ! $this->get_theme_option_name() ) {
				wp_die( esc_html__( 'Theme options name not defined. Please define it and try again.', 'school-time' ), '', array( 'back_link' => true ) );
			}
