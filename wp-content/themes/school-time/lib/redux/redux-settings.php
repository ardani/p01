...
function school_time_redux_remove_demo_mode_link() {
...
add_action('init', 'school_time_redux_remove_demo_mode_link');
...
function class_compiler_action( $options, $css, $changed_values ) {
	...
	$filename   = $upload_dir['basedir'] . '/' . SCHOOL_TIME_THEME_OPTION_NAME . '_style.css';
...
//	}
	...
	// Other Seettings Vars
	 ...
	 if (isset($options[$option_name])) {
		$scssphp_filepath = WP_PLUGIN_DIR . '/' . str_replace( '_', '-', SCHOOL_TIME_THEME_NAME ) . '-plugin/extensions/scssphp/scss.inc.php';
		 if (version_compare(phpversion(), '5.3.10', '>=') && file_exists( $scssphp_filepath ) ) {
			 ...
			 $buffer = school_time_strip_comments( $buffer );
			 $lines = '';
			 if ( $buffer ) {
			 ...
			 try {
				 ...
				 $data = school_time_strip_comments( $options[$option_name] . "\n" );
	...
	if (is_writable($upload_dir['basedir'])) {
		...
		wp_die(esc_html__("It looks like your upload folder isn't writable, so PHP couldn't make any changes (CHMOD).", 'school-time'), esc_html__('Cannot write to file', 'school-time'), array('back_link' => true));
	}
...
$args = array(
	...
	'menu_title'           => esc_html__( 'Theme Options', 'school-time' ),
	'page_title'           => esc_html__( 'Theme Options', 'school-time' ),
