				...
				<div class="teacher <?php echo $grid_class; ?> <?php echo $teacher_id ? '' : 'wh-padding'; ?>">
					<div class="thumbnail">
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$img_url = get_the_post_thumbnail( $post->ID, $image_size );
						}
						if ( '' != $img_url ) {
					...
					<div class="item">
						<h5 class="entry-title"><a href="<?php the_permalink($post); ?>"><?php echo $post->post_title; ?></a>
						</h5>
						<?php $job_title = school_time_get_rwmb_meta( 'job_title', $post->ID ); ?>
						<?php if ( $job_title ) : ?>
							<div class="job-title"><?php echo $job_title; ?></div>
						<?php endif; ?>
						<?php $summary = school_time_get_rwmb_meta( 'summary', $post->ID ); ?>
						<?php if ( $summary ) : ?>
							<div
								class="summary"><?php echo wp_trim_words( do_shortcode( $summary ), $description_word_length ); ?></div>
						<?php else: ?>
						...
						<?php if ( $social ) : ?>
							<div class="social">
								<div class="text"><?php esc_html_e( 'Meet me on:', 'school_time' ); ?></div>
